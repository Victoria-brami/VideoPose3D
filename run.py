...
dataset_path = '/datasets_local/DriveAndAct/data_3d_' + args.dataset + '_train.npz'
if args.dataset == 'h36m':
    ...
    dataset = CustomDataset('/datasets_local/DriveAndAct/data_2d_' + args.dataset + '_' + args.keypoints + '.npz')
else:
    ...
    for subject in subjects:
        for i, action in enumerate(keypoints[subject].keys()):
            ...
            for i in range(len(poses_2d)): # Iterate across cameras
                out_poses_2d.append(poses_2d[i][args.seq_start: args.seq_start + args.seq_length])
...
if args.resume or args.evaluate:
    ...
    if args.evaluate and checkpoint["model_traj"] is not None:
...
if not args.evaluate:
    ...
    writer = SummaryWriter(log_dir=args.tb_logs)
    ...
    while epoch < args.epochs:
        ...
        if semi_supervised:
            ...
            for (_, batch_3d, batch_2d), (cam_semi, _, batch_2d_semi) in \
                ...
                writer.add_scalar('Train/Semi/Loss_3d_pose', loss_3d_pos, epoch * train_generator.num_frames() + batch_id)
                ...
                # Add logs to tensorboard
                # print("Trajectory Loss: ", loss_traj)
                writer.add_scalar('Train/Semi/Loss_trajectory', loss_traj, epoch * train_generator.num_frames() + batch_id)
                ...
                if not skip:
                    ...
                    if args.bone_length_term:
                        ...
                        loss_total += penalty
                        # Add logs to tensorboard
                        writer.add_scalar('Train/Semi/Loss_bones_length_penalty', penalty, epoch * train_generator.num_frames() + batch_id)
                    ...
                    N_semi += predicted_semi.shape[0]*predicted_semi.shape[1]
                     # Add logs to tensorboard
                    writer.add_scalar('Train/Semi/Loss_reconstruction', epoch_loss_2d_train_unlabeled / N_semi, epoch * train_generator.num_frames() + batch_id)
            ...
            print("N semi and loss unlabeled: ", N_semi, epoch_loss_2d_train_unlabeled)
            losses_2d_train_unlabeled.append(epoch_loss_2d_train_unlabeled / N_semi)
            
            # Add logs to tensorboard
            writer.add_scalar('Train/Semi/Epoch_loss_traj_train', epoch_loss_traj_train / N, epoch)
            writer.add_scalar('Train/Semi/Epoch_loss_2d_unlabeled', epoch_loss_2d_train_unlabeled / N_semi, epoch)
            ...
            for _, batch_3d, batch_2d in train_generator.next_epoch():
                ...
                if args.bone_symmetry_term:
                    dists = predicted_3d_pos[:, :, 1:] - predicted_3d_pos[:, :, dataset.skeleton().parents()[1:]]
                    bone_lengths = torch.mean(torch.norm(dists, dim=3), dim=1)
                    penalty = args.lambda_sym * torch.mean(torch.abs(torch.mean(bone_lengths[[0,2,4,6,8,10,12,14]], dim=0) - torch.mean(bone_lengths[[1,3,5,7,9,11,13,15]], dim=0)))# replace by left kpts
                    loss_total += penalty
                    # Add logs to tensorboard
                ...
                if args.illegal_angle_term:
                    # Compute the vectors associated with bones
                    up_left_loss, up_right_loss, low_left_loss, low_right_loss, head_loss = angle_error(predicted_3d_pos, debug=args.debug)

                    # Compute the angle between these vectors
                    """  
                    angles_penalty = torch.mean(up_left_loss * torch.exp(up_left_loss) \
                                        + up_right_loss * torch.exp(up_right_loss) \
                                        + low_left_loss * torch.exp(low_left_loss) \
                                        + low_right_loss * torch.exp(low_right_loss) \
                                        + head_loss * torch.exp(head_loss))
                    """  
                    angles_penalty = args.lambda_angle * torch.mean(head_loss * torch.exp(head_loss)) \
            ...
            if not args.no_eval:
                ...
                for cam, batch, batch_2d in test_generator.next_epoch():
                    ...
                    if semi_supervised:
                        ...
                        # Add logs to tensorboard
                        writer.add_scalar('Validation/Semi/Loss_trajectory', epoch_loss_traj_valid / N, epoch * test_generator.num_frames() + val_batch_id)
                        ...
                        # Add logs to tensorboard
                        writer.add_scalar('Validation/Semi/Loss_reconstruction', loss_reconstruction, epoch*test_generator.num_frames() + val_batch_id)
                    val_batch_id += 1
                losses_3d_valid.append(epoch_loss_3d_valid / N)
                # Add logs to tensorboard
                writer.add_scalar('Validation/Epoch_Loss', epoch_loss_3d_valid / N, epoch)

                if semi_supervised:
                    losses_traj_valid.append(epoch_loss_traj_valid / N)
                    losses_2d_valid.append(epoch_loss_2d_valid / N)
                    
                    writer.add_scalar('Validation/Semi/Epoch_Loss_2d', epoch_loss_2d_valid / N, epoch)
                    writer.add_scalar('Validation/Semi/Epoch_Loss_2d_traj', epoch_loss_traj_valid / N, epoch)
                ...
                writer.add_scalar('Train_inference/Epoch_Loss_3d', epoch_loss_3d_train_eval / N, epoch)
                if semi_supervised:
                    ...
                    writer.add_scalar('Train_inference/Semi/Epoch_Loss_2d', epoch_loss_2d_train_labeled_eval / N, epoch)
                    writer.add_scalar('Train_inference/Semi/Epoch_Loss_2d_traj', epoch_loss_traj_train_eval / N, epoch)
                ...
                if semi_supervised:
                    for cam, _, batch_2d in semi_generator_eval.next_epoch():
                        ...
                        writer.add_scalar('Train_inference/Semi/Epoch_Loss_2d', epoch_loss_2d_train_unlabeled_eval / N_semi, epoch*train_generator_eval.num_frames() + batch_id)
                        batch_id += 1
...
if args.render:
    ...
    input_keypoints = keypoints[args.viz_subject][args.viz_action].copy() # removed last key [args.viz_camera]
    ground_truth = None
